...
int32_t sensor_1mV = 0, sensor_100uA = 0;

void sendData() {
  sensor_1mV   = INA219_getBusVoltage();
  sensor_100uA = INA219_getCurrent() / 100;
  ...
  if ( Serial1.available() > 0 ) {
    ...
    if ( ch == '\n' ) {
      ...
      if ( !strcmp(line_buf, "ARM") ) {
        ...
        setTime = false;
        INTERVAL_USEC = atoi(line_buf);
        Serial.println(INTERVAL_USEC);
      } else if (setCount) {
        // convert line_buf to int
        setCount = false;
        NUM_SAMPLES = atoi(line_buf);
